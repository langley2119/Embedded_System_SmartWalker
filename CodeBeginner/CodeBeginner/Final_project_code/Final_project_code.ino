...
void setup() {
  ...
  pinMode(interruptButtonLeft, INPUT); // need to configure for button interrupt
  attachInterrupt(digitalPinToInterrupt(interruptButtonLeft), thank_you, RISING);
  Serial.begin(9600);
  while (!Serial);
...
unsigned int distance_lower = 0;
  switch (flag){
    ...
    break; // need breaks after every case so this can execute properly
