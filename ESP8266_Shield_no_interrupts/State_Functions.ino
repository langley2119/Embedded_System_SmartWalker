...
{ 
  ...
  static bool prev_button_press = false; 
  static bool current_button_press = false; 
  counter = counter + 1;
  time_elapsed = counter * STATE_MACH_INTERVAL;  
  
  // cycle the button states
  prev_button_press = current_button_press; 
  
  // read the buttons 
  if(digitalRead(RIGHT_HANDLE_BUTTON) == true || digitalRead(LEFT_HANDLE_BUTTON) == true){
    //DEBUG.println("a button detected");
    current_button_press = true;   
  } else {
    current_button_press = false; 
  }
  if(current_button_press == true && prev_button_press == false){
    DEBUG.println("a rising button edge!");
    if(current_state != thankYou)
      next_state = thankYou; // set the next state as an override. 
    // if the buttons are pressed it is assumed the user is using the walker and should be thanked. 
  }
